...
class coord_policy_spherical_base_bounce {
 public:
  using value_t = typename Conf::value_t;
  using grid_type = grid_sph_t<Conf>;

  coord_policy_spherical_base_bounce(const grid_t<Conf>& grid) :
      m_grid(dynamic_cast<const grid_type&>(grid)),
      m_pusher(grid) {}
  ~coord_policy_spherical_base_bounce() = default;
  ...
  HD_INLINE void move_ptc(const Grid<Conf::dim, value_t>& grid,
    ...
    if(x_global_sph_new[0] <= 0.0f) {
      ...
      x_global_sph_new[0] = log(2.0-math::exp(x_global_sph_new[0]));
...
using coord_policy_spherical_bounce = coord_policy_spherical_base_bounce<Conf, pusher_simple>;
