...
class coord_policy_spherical_gca : public coord_policy_spherical<Conf> {
 public:
  ...
  HOST_DEVICE void update_ptc(const Grid<Conf::dim, value_t>& grid,
    ...
    //value_t u_par_new = context.p[0] + context.q * dt * E_par / context.m;
    context.p[0] = context.p[0]+context.q*dt*E_par/context.m;
    // Compute the new Lorentz factor, since it depends on the new u_par
    value_t Gamma_new = f_Gamma(context.p[0], context.p[1], B_mag, vE);
    // Need to update this because we are going to use this in the iteration
    context.gamma = Gamma_new;
    //context.gamma = f_Gamma(context.p[0],context.p[1],B_mag,vE);
    ...
    for (int i = 0; i < n_iter; i++) {
      ...
      move_ptc_gca(grid, x_iter, b, B_iter, vE, vE_iter,
                 Gamma_new, context.gamma, context.p[0], dt);
      ...
      E_iter[0] =
          interp(context.new_x, m_E[0], idx_iter, ext);
      E_iter[1] =
          interp(context.new_x, m_E[1], idx_iter, ext);
      E_iter[2] =
          interp(context.new_x, m_E[2], idx_iter, ext);
      B_iter[0] =
          interp(context.new_x, m_B[0], idx_iter, ext);
      B_iter[1] =
          interp(context.new_x, m_B[1], idx_iter, ext);
      B_iter[2] =
          interp(context.new_x, m_B[2], idx_iter, ext);
      vE_iter = f_v_E(E_iter, B_iter);
      B_mag_iter = math::sqrt(B_iter.dot(B_iter));
      B_iter /= B_mag_iter;
      context.gamma = f_Gamma(context.p[0], context.p[1], B_mag_iter, vE_iter);
    }
